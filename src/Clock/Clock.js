...
class Clock extends React.Component { // same as function name + extends React.Component
  constructor(props) { // always pass props to constructor, to pass use in super()
  ...
  componentDidMount() { // lifecycle hook called to update state after first render
  ...
  tick() {
    if (this.state.isToggleOn === false){
      this.setState({ // setState = render again
        date: new Date(),
        seconds: new Date()
      });
    }
  ...
  render() { // Renders after constructor and calls componentDidMount after seeing 'state'
    ...
    return ( // call this.state to indicate update
      <div className="timers row">
        ...
        <div className="clock-buttons">
          ...
          <button className="btn btn-primary btn-sm" onClick={this.zeroSecs.bind(this, "counter")}>Reset sec counter</button>
        </div>
      </div>
    );
  }
